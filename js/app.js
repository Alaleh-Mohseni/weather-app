//  ApiKey
const apiKey = "e4dd7b2139ae4bdbaf7130638231909";
//  header
const cityInNav = document.querySelector("#city-in-nav");
const cities = ["London", "doha", "Toronto", "paris"];
// geolocation
...
function loadNavApi(cities) {
    return cities.forEach((query) => {
        const fetchData = fetch(`http://api.weatherapi.com/v1/current.json?key=${apiKey}&q=${query}`)
            .then((response) => response.json())
            .then((data) => {
                cityInNav.innerHTML += `
            <li class="nav-item border-right-light py-2 col-3 border-end">
...
function currentWeather(data, temp, cur) {
    ...
    cur.innerHTML = `<div class="container-fluid row">
...
function searchCity() {
    ...
    const api = fetch(`http://api.weatherapi.com/v1/forecast.json?key=${apiKey}&q=${inputVal}&day=1&aqi=no&alerts=yes`)
        .then((response) => response.json())
        .then((data) => {
            currentWeather(data, dayBox, currentBox);
            const nameCity = data.location.name;
            const country = data.location.country;
            const localTime = data.location.localtime;
            ...
            hourlyWeather.innerHTML = `
                      <div class="ps-2 border-bottom" id="hourlyHeader">
                          ...
                          <span>-${nameCity}, ${nameCity}, ${country}</span>
                          <p class="pt-2 mb-3">
                              As of ${localTime}
                          </p>
                      </div>
